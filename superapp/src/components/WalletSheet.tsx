...
export function WalletSheet({ wallet, onClose, onUpdated, onNavigate, locale }: { wallet: Wallet | null; onClose: () => void; onUpdated: () => void; onNavigate: (tab: 'shop' | 'games' | 'orders' | 'profile') => void; locale: Locale }) {
  ...
  const [copied, setCopied] = useState<{ type: 'card' | 'amount'; id?: string } | null>(null);
  ...
  async function copyValue(value: string, kind: 'card' | 'amount', id?: string) {
    try {
      await navigator.clipboard.writeText(value);
      setCopied({ type: kind, id });
      window.setTimeout(() => setCopied((current) => current?.type === kind && current.id === id ? null : current), 1800);
    } catch { setError(locale === 'ru' ? 'Не удалось скопировать. Выделите текст и скопируйте вручную.' : "Karta raqamini belgilab nusxalang."); }
  }
  ...
  return <Sheet title={t("Balansni to'ldirish")} onClose={onClose} busy={busy} className="wallet-sheet">
    <div className="checkout-form">
      ...
      {request?.status === 'VERIFIED' ? <div className="checkout-result"><CheckCircle2 size={44}/><h3>{t("Balans to'ldirildi")}</h3><p>{money(request.amountMinor, 'UZS', locale)}</p><button className="button primary" onClick={onClose}>{t('Tayyor')}</button></div> : request ? <>
        <div className="topup-total"><span className="eyebrow">{t("TO'LOV SUMMASI")}</span><div className="copyable-amount"><h3 className="amount-display">{money(request.amountMinor, 'UZS', locale)}</h3><button className={`copy-button ${copied?.type === 'amount' ? 'copied' : ''}`} aria-label={locale === 'ru' ? 'Скопировать сумму' : 'Summani nusxalash'} title={copied?.type === 'amount' ? t('Nusxalandi') : t('Nusxalash')} onClick={() => void copyValue(String(request.amountMinor / 100), 'amount')}>{copied?.type === 'amount' ? <Check size={18}/> : <Copy size={18}/>}<span>{copied?.type === 'amount' ? t('Nusxalandi') : t('Nusxalash')}</span></button></div></div>
